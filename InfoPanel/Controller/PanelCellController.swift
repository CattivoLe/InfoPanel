...
class PanelCellController: UITableViewCell {
    ...
    func setValue(currentPanel: Panel) {
        
        nameLabelCell.text = currentPanel.name
        
        switch currentPanel.group {
        ...
        guard let image = currentPanel.image else {return}
        if currentPanel.orient == .vertical {
